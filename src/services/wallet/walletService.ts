...
import { DEFAULT_CHAINS, EVM_CHAINS } from '../../providers/blockradar/walletIdAndTokenManagement/chainsAndTokensHelpers';
...
export class WalletService {
  ...
  static async generateMultiChainWallets(
    ...
    try {
      ...
      const evmChainsList = Array.from(EVM_CHAINS) as readonly string[];
      const evmChains = chains.filter(chain => evmChainsList.includes(chain as string));
      const nonEvmChains = chains.filter(chain => !evmChainsList.includes(chain as string));
      ...
      const walletDataPromises = chains.map(async (chain) => {
        ...
        if (evmChainsList.includes(chain as string)) {
